...
$query = $pdo->prepare('SELECT id_cat, cat_name, is_active FROM category_comics JOIN comics USING (id_comics) JOIN category USING (id_cat) WHERE id_type = 2 GROUP BY id_cat, cat_name, is_active');
$query->execute();
...
$query = $pdo->prepare('SELECT COUNT(id_comics) AS count FROM comics WHERE id_type = 2');
$query->execute();
$count = $query->fetch();
include_once 'bd.phtml';
